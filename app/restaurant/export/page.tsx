...
<body>
  <h1>${context_pack.metadata.name} - Stress Test Report</h1>
  ...
  <div class="grid">
    ...
    <div class="metric">
      <div>Net Profit</div>
      <div class="metric-value ${totalNetProfit < 0 ? 'negative' : 'positive'}">${formatCurrency(totalNetProfit)}</div>
    </div>
    <div class="metric">
      <div>Avg Gross Margin</div>
      <div class="metric-value">${formatPercent(context_pack.derived_summary.gross_margin_pct.mean)}</div>
    </div>
    <div class="metric">
      <div>Avg Prime Cost</div>
      <div class="metric-value">${formatPercent(context_pack.derived_summary.prime_cost_pct.mean)}</div>
    </div>
  </div>
  
  <h2>Key Assumptions (${assumption_set?.assumptions.length || 0})</h2>
  <table>
    <thead>
      <tr>
        <th>ID</th>
    ...
    <tbody>
      ${(assumption_set?.assumptions || []).map(a => `
        <tr>
          <td>${a.id}</td>
          <td>${a.label}</td>
  ...
  <h2>Stress Scenarios (${scenario_set?.scenarios.length || 0})</h2>
  <table>
    <thead>
      <tr>
        <th>ID</th>
  ...
  <h2>Mitigations (${mitigation_set?.mitigations.length || 0})</h2>
  <table>
    <thead>
      <tr>
        <th>ID</th>
    ...
    <tbody>
      ${(mitigation_set?.mitigations || []).map(m => `
        <tr>
          <td>${m.id}</td>
          <td>${m.name}</td>
          <td>${m.scenario_id}</td>
  ...
  <div class="footer">
    ...
    <p>Dataset ID: ${context_pack.metadata.dataset_id} | Context Pack: ${context_pack.id}</p>
    <p>Assumption Set Version: ${assumption_set?.version || 'N/A'} | Scenario Set Version: ${scenario_set?.version || 'N/A'}</p>
  </div>
</body>
</html>`
  }, [state])
  ...
  const handleDownloadJSON = () => {
    ...
    a.href = url
    a.download = `${context_pack.metadata.name.replace(/\s+/g, '_')}_stress_report.json`
    a.click()
  ...
  const handleDownloadHTML = () => {
    ...
    const a = document.createElement('a')
    a.href = url
    a.download = `${context_pack.metadata.name.replace(/\s+/g, '_')}_stress_report.html`
    a.click()
    URL.revokeObjectURL(url)
  }
